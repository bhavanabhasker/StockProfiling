...
func (t *Api) Buyingstocks(args *Args, replies *Reply) error {
	...
	if args.Trade == 0 {
		...
		for i := 0; i < len(incom); i++ {
			...
			con := url(stocksym[0])
			//http get to get the data from yahoo api
			...
			if err == nil {
				...
				if err == nil {
					...
					ask := (buff).(string)
					//calculations to purchase stocks
		...
		// store the response in hash maps
		hash[replies.Trade] = Values{replies.Stocks, replies.Unvested}
	}
	return nil
}
func (t *Api) Checkportfolio(args *Args, replies *Reply) error {
...
//Function to get the url for HTTP GET
func url(sym string) string {
